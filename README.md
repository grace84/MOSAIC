...
```
```math
\mathbf{Z}_m \sim \text{Multinomial}(\boldsymbol{\lambda}_m) \text{ with } \boldsymbol{\lambda} = [\lambda_{1}, \ldots, \lambda_{K}]^{\top}, \lambda_{k} \ge 0 \text{ and } \sum_{k=1}^K \lambda_{k} = 1.
```
